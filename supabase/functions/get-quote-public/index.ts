...
Deno.serve(async (req) => {
  ...
  const { data, error } = await supabaseAdmin.from('quotes').select('*').eq('share_token', shareToken).single()
  if (error || !data) return json({ error: 'Quote not found or link expired.' }, 404, headers)
