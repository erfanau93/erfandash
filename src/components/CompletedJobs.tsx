...
export default function CompletedJobs() {
  ...
  const filteredAndSortedRows = useMemo(() => {
    ...
    return sorted
  }, [combinedRows, pastDueRows, paymentFilter, sortBy, getAmountCentsForRow])

  const hasAnyRows = rows.length > 0 || pastDueRows.length > 0
    ...
    <div className="min-h-screen p-6 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          >
            <p className="text-sm text-[var(--color-text-muted)]">All jobs</p>
            <p className="text-3xl font-bold text-white">{combinedRows.length}</p>
          </button>
          <button
